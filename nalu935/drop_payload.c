/*
 *
 * Put any binaries you want to run in a folder called iosbinpack and drag-and-drop
...
char* prepare_directory(char* dir_path) {
  ...
  asprintf(&in_path, "%s/iosbinpack/%s", bundle_root, dir_path);
  ...
  while ((ep = readdir(dp))) {
    ...
    asprintf(&full_entry_path, "%s/iosbinpack/%s/%s", bundle_root, dir_path, entry);
...
// prepare all the payload binaries under the iosbinpack directory
...
void do_bind_shell(char* env, int port) {
  ...
  asprintf(&shell_path, "%s/iosbinpack/bin/bash", bundle_root);
