...
uint64_t amfid_MISValidateSignature_import_offset = 0x7a54;
uint64_t amfid_CFDataCreateWithBytesNoCopy_import_offset = 0x7a90;

uint64_t amfid_MISCopyErrorStringForErrorCode_import_offset = 0x7a18;
...
void* amfid_exception_handler(void* arg){
  ...
  for(;;){
    ...
    if (err != KERN_SUCCESS){
      ...
      new_state.__pc = amfid_base + 0x744e; // where to continue
...
int patch_amfid(mach_port_t amfid_task_port) {
  ...
  w64(amfid_task_port, amfid_base+amfid_MISValidateSignature_import_offset, 0x41414140); // crashy
